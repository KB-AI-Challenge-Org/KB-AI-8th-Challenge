...
type Screen = 'reception' | 'documents'
...
type ResponseStage = 'typing' | 'guide' | 'warning'
...
function AssistantPanel({
  ...
  const isResponding = conversation !== null && responseStage !== 'warning'
  ...
  useEffect(() => {
    ...
    if (responseStage === 'typing') {
      const timer = window.setTimeout(() => setResponseStage('guide'), 1_150)
      return () => window.clearTimeout(timer)
    }

    if (responseStage === 'guide') {
      const timer = window.setTimeout(() => setResponseStage('warning'), 520)
      return () => window.clearTimeout(timer)
    }
  }, [conversation, responseStage])
    ...
    >
      ...
      <div className="panel-bottom">
        <div className="quick-actions">
          {screen === 'reception' ? (
            <button type="button" onClick={onOpenDocuments}>서류 안내로 이동하기</button>
          ) : (
            <button type="button">서류 체크 완료</button>
          )}
