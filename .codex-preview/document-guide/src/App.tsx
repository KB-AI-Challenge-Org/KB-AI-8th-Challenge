...
function AssistantPanel({
  ...
  return (
    ...
    >
      ...
      <div className="chat-area" key={screen}>
        <div className="user-message">
          <span className="speaker">나</span>
          <div className="message-bubble">
            <p>법인 해지 접수 시 주의사항을 알려줘.</p>
            <time>오전 09:05 ✓✓</time>
          </div>
        </div>
        ...
        {screen === 'reception' ? (
          <article className="assistant-card">
            ...
            <div className="reference-chips">
              <button>법인 인감증명서 안내</button>
              <button>위임장 작성 예시</button>
            </div>
          </article>
        ) : (
          <>
            <article className="assistant-card document-assistant-card">
              ...
              <time className="assistant-time">오전 09:05</time>
            </article>
      ...
      <div className="panel-bottom">
        <div className="quick-actions">
          {screen === 'reception' ? (
            <button onClick={onOpenDocuments}>서류 안내로 이동하기</button>
          ) : (
            <button>서류 체크 완료</button>
          )}
          <button>유사 사례 더보기</button>
        </div>
        <label className="chat-input">
          <input aria-label="도우미에게 질문" placeholder="궁금한 내용을 입력해 주세요." />
          <button aria-label="질문 보내기">↑</button>
        </label>
        <div className="report-row"><button>신고하기</button></div>
