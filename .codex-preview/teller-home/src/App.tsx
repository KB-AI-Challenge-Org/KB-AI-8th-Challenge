...
type Screen = 'home' | 'reception' | 'documents'
...
function useKoreaTime(): KoreaTime {
  ...
  return {
    ...
    date: koreaDateFormatter.format(now),
  }
}
...
const dashboardTabs = ['홈', '수신', '여신', '외환', '카드', '신탁', '고객관리', '공통']

const quickTasks = [
  { label: '고객 조회', description: '고객번호·사업자번호 조회', shortcut: 'F4' },
  { label: '예금 신규', description: '개인·기업 예금 신규', shortcut: 'F6' },
  { label: '예금 해지', description: '만기·중도해지 접수', shortcut: 'F7' },
  { label: '계좌 이체', description: '당행·타행 이체 처리', shortcut: 'F9' },
  { label: '제신고', description: '인감·비밀번호·분실 신고', shortcut: 'F10' },
  { label: '증명서 발급', description: '잔액·거래내역 증명', shortcut: 'F11' },
]
...
function HomeWorkspace({
  ...
  return (
    ...
    >
      <nav className="tab-nav home-tabs" aria-label="업무 메뉴">
        {dashboardTabs.map((tab, index) => (
          <button className={index === 0 ? 'tab active' : 'tab'} key={tab}>{tab}</button>
        ))}
      </nav>

      <div className="breadcrumb-row">
        <div className="breadcrumbs"><span>홈</span><i>›</i><strong>업무 대시보드</strong></div>
        <div className="function-keys"><span>F1 도움말</span><span>F4 고객조회</span><span>F12 전체메뉴</span></div>
      </div>

      <div className="home-content">
        <header className="home-welcome">
          <div>
            <span className="home-eyebrow">MY WORK DESK</span>
            <h1>최영환 행원님, 오늘 업무를 시작해 볼까요?</h1>
            <p>진행 중인 업무와 확인이 필요한 항목을 먼저 살펴보세요.</p>
          </div>
          <div className="home-date-card">
            <span>잠실종합금융센터</span>
            <time dateTime={koreaTime.iso}>{koreaTime.date}</time>
          </div>
        </header>

        <section className="work-summary" aria-label="오늘의 업무 현황">
          <article><span className="summary-marker waiting" aria-hidden="true" /><div><small>대기 고객</small><strong>4</strong></div><p>기업 고객 2명 포함</p></article>
          <article><span className="summary-marker progress" aria-hidden="true" /><div><small>진행 중 업무</small><strong>1</strong></div><p>중도해지 접수 작성 중</p></article>
          <article><span className="summary-marker check" aria-hidden="true" /><div><small>확인 필요</small><strong>2</strong></div><p>서류·승인 항목 확인</p></article>
        </section>

        <div className="home-main-grid">
          <section className="home-panel ongoing-panel">
            <header className="home-panel-title">
              <div><span className="title-accent" aria-hidden="true" /><h2>진행 중 업무</h2></div>
              <span>최근 업데이트 {koreaTime.clock}</span>
            </header>
            <button className="ongoing-work-card" type="button" onClick={onStart}>
              <span className="work-type">수신</span>
              <span className="work-detail">
                <strong>(주)OO물류</strong>
                <b>기업 정기예금 중도해지 접수</b>
                <small>계좌 812-****-****-21 · 예치금액 300,000,000원</small>
              </span>
              <span className="work-status"><small>SD-3412</small><em>입력 중</em></span>
              <span className="continue-action">이어서 처리 <i aria-hidden="true">→</i></span>
            </button>
          </section>

          <aside className="home-panel notice-panel">
            <header className="home-panel-title">
              <div><span className="title-accent" aria-hidden="true" /><h2>오늘의 안내</h2></div>
            </header>
            <ul>
              <li><span>업무</span><p>법인 고객 실명확인 절차 개정 안내</p></li>
              <li><span>보안</span><p>고액 현금거래 보고 기준 재확인</p></li>
              <li><span>지점</span><p>잠실점 기업고객 예약 상담 2건</p></li>
            </ul>
          </aside>
        </div>
        ...
        <section className="quick-task-section">
          <header className="home-panel-title">
            <div><span className="title-accent" aria-hidden="true" /><h2>빠른 업무 시작</h2></div>
            <span>자주 사용하는 업무</span>
          </header>
          <div className="quick-task-grid">
            {quickTasks.map((task) => (
              <button type="button" key={task.label}>
                <span><strong>{task.label}</strong><small>{task.description}</small></span>
                <kbd>{task.shortcut}</kbd>
              </button>
            ))}
          </div>
        </section>
...
function MainWorkspace() {
  return (
    <section className="workspace workspace-entry">
      <nav className="tab-nav" aria-label="업무 메뉴">
        {tabs.map((tab, index) => (
          <button className={index === 0 ? 'tab active' : 'tab'} key={tab}>
            {tab}
...
function App() {
  ...
  const appBodyClass = screen === 'home'
    ? 'app-body home-mode'
    : isPanelOpen ? 'app-body' : 'app-body panel-closed'
  ...
  return (
    <main className="banking-app">
      ...
      <div className={appBodyClass}>
        {screen === 'home' ? (
          <HomeWorkspace onStart={startReception} koreaTime={koreaTime} />
        ) : screen === 'reception' ? (
          <MainWorkspace />
        ) : (
          <DocumentWorkspace onBack={() => setScreen('reception')} />
        )}
        {screen !== 'home' && !isPanelOpen && (
          <button
            type="button"
            ...
            title="S 패널 열기"
          >
            <svg className="panel-open-mark" viewBox="0 0 48 48" aria-hidden="true">
